...
function func_init_hbase
{
    # Check whether hbase is init
    is_hbase_init=$BASE_DIR/Initialized
    if [ -f $is_hbase_init ] && [[ $(cat $is_hbase_init) == "true" ]]; then
        echo "Hbase is already initialized"
    else
        echo "Hbase is NOT initialized..."
        echo "Init Hbase..."
        $BASE_DIR/scripts/init-hbase.sh
        echo "true" > $is_hbase_init
        echo "Hbase initialization done"
    fi
