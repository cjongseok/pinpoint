...
ENV PINPOINT_VERSION=1.6.0-SNAPSHOT
...
# Copy and Extract Pinpoint war files
RUN set -u \
        && mkdir -p ${PINPOINT_HOME}/collector ${PINPOINT_HOME}/web ${PINPOINT_CONF_DIR}
COPY ./bin/pinpoint-collector-1.6.0-SNAPSHOT.war ${PINPOINT_HOME}/collector/
COPY ./bin/pinpoint-web-1.6.0-SNAPSHOT.war ${PINPOINT_HOME}/web/
RUN set -u \
        && cd ${PINPOINT_HOME}/collector \
        && unzip pinpoint-collector-${PINPOINT_VERSION}.war \
        && rm pinpoint-collector-${PINPOINT_VERSION}.war \
...
# Download and extract Hbase for using Hbase shell to check whether tables exist on Hbase
